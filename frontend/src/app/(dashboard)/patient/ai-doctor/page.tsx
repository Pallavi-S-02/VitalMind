    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] bg-[#040814] rounded-3xl overflow-hidden relative shadow-2xl border border-white/5 ring-1 ring-white/10 font-sans">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
        ...
        <div className={`absolute -top-32 -left-32 w-[40rem] h-[40rem] rounded-full blur-[120px] opacity-30 mix-blend-screen transition-all duration-[2000ms] ease-in-out ${
          avatarState === "listening" ? "bg-blue-600 scale-110" : avatarState === "speaking" ? "bg-emerald-600 scale-100 translate-x-10" : "bg-cyan-900 scale-90"
        }`} />
        ...
        <div className={`absolute -bottom-32 left-1/3 w-[50rem] h-[20rem] rounded-full blur-[100px] opacity-20 mix-blend-screen transition-colors duration-[3000ms] ${
          isRecording ? "bg-rose-900" : "bg-blue-900"
        }`} />
      ...
      <div className="relative z-10 flex flex-1 overflow-hidden h-full">
        ...
        {/* ─── Left Panel: Avatar & HUD Controls ─── */}
        <div className="flex flex-col items-center py-10 px-8 w-[340px] flex-shrink-0 border-r border-white/5 bg-[#091124]/30 backdrop-blur-md relative">
          ...
          <div className="flex-1 w-full flex flex-col items-center justify-center relative">
            ...
            <div className={`absolute top-0 flex items-center gap-2 px-4 py-1.5 rounded-full border text-[10px] uppercase tracking-[0.2em] font-black transition-all duration-500 ${
              avatarState === "speaking" ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.2)]" :
              avatarState === "listening" ? "bg-blue-500/10 border-blue-500/30 text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.2)]" :
              "bg-slate-800/50 border-slate-700/50 text-slate-400"
            }`}>
              {avatarState === "speaking" ? <Volume2 className="w-3 h-3" /> :
               avatarState === "listening" ? <Mic className="w-3 h-3" /> : 
               <Activity className="w-3 h-3" />}
              {avatarState === "speaking" ? "AI Transmitting" :
               avatarState === "listening" ? "Acoustic Sensors Active" : 
          ...
          <div className="w-full space-y-4 relative z-20">
            ...
            {sharedWithDoctor && (
              <div className="flex items-center gap-3 bg-emerald-950/50 border border-emerald-500/40 rounded-2xl p-4 shadow-[0_0_20px_rgba(16,185,129,0.15)] animate-in slide-in-from-bottom-4">
        ...
        {/* ─── Middle Panel: Conversation Real-Time Feed ─── */}
          ...
          <div className="px-8 py-4 border-b border-white/5 flex items-center gap-4 bg-gradient-to-b from-[#091124] to-transparent">
            ...
            {turns.length > 0 && (
              <div className="ml-auto flex flex-col items-end">
                 <span className="text-xl font-light text-cyan-400 font-mono">{Math.ceil(turns.length / 2)}</span>
          ...
          <div ref={transcriptRef} className="flex-1 overflow-y-auto px-8 py-8 space-y-6 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10 z-10">
            {turns.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center gap-6 opacity-60 max-w-md mx-auto animate-in zoom-in-95 duration-[1s]">
                ...
                <div className="space-y-2">
                  <h2 className="text-2xl font-light text-white tracking-widest font-outfit">Awaiting Input</h2>
                  <p className="text-sm text-slate-400 font-medium leading-relaxed px-6">
                    Voice recognition active. Speak clearly into your device microphone to detail your symptoms.
                  </p>
                </div>
              </div>
            ) : (
            ...
            {isRecording && localTranscript && (
              <div className="flex justify-end animate-in fade-in slide-in-from-bottom-2 duration-300">
                <div className="max-w-[75%] px-6 py-4 rounded-3xl rounded-tr-sm bg-blue-500/5 border border-blue-500/20 text-blue-200/60 text-[15px] italic shadow-[0_0_20px_rgba(59,130,246,0.05)] flex flex-col gap-3">
                  {localTranscript}
                  <div className="w-full h-px bg-gradient-to-r from-blue-500/0 via-blue-500/30 to-blue-500/0" />
                </div>
              </div>
            )}

            {/* Waiting for audio UI */}
            {isRecording && !localTranscript && (
              <div className="flex justify-end">
                <div className="px-6 py-4 rounded-3xl rounded-tr-sm bg-blue-500/5 border border-blue-500/20 flex gap-2 items-center">
                  ...
                  <span className="text-[10px] uppercase font-bold text-blue-400 ml-2 tracking-wider">Listening</span>
                </div>
              </div>
            )}
          </div>
